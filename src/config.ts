...
export function validateConfig(config: FetcherConfig): { valid: boolean; errors: string[] } {
  ...
  if (config.initialRetryDelay !== undefined && (config.initialRetryDelay < 100 || config.initialRetryDelay > 30000)) {
    errors.push('initialRetryDelay must be between 100ms and 30 seconds');
  }
  
  if (config.rateLimitPerSecond !== undefined && (config.rateLimitPerSecond < 1 || config.rateLimitPerSecond > 1000)) {
    errors.push('rateLimitPerSecond must be between 1 and 1000');
  }
  
  if (config.maxLogsPerChunk !== undefined && (config.maxLogsPerChunk < 1 || config.maxLogsPerChunk > 1000000)) {

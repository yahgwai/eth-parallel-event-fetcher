...
describe('Integration - Parallel Processing', () => {
  ...
  it('should fetch events using multiple parallel chunks', async () => {
    ...
    const events = await fetcher.fetchEvents(usdcContract, 'Transfer', {
      ...
      fromBlock: 18500000,
      toBlock: 18506000,
      onProgress: (completed, total, _chunk) => {
    ...
    expect(progressUpdates[progressUpdates.length - 1].completed).toBe(
  ...
  it('should maintain event order across chunks with high concurrency', async () => {
    ...
    const events = await customFetcher.fetchEvents(usdcContract, 'Transfer', {
      contractAddress: usdcContract.address,
  ...
  it('should handle different chunk sizes correctly', async () => {
    ...
    const [smallChunkEvents, largeChunkEvents] = await Promise.all([
      smallChunkFetcher.fetchEvents(usdcContract, 'Transfer', {
        contractAddress: usdcContract.address,
        fromBlock: 18500000,
        toBlock: 18501000,
      }),
      largeChunkFetcher.fetchEvents(usdcContract, 'Transfer', {
        contractAddress: usdcContract.address,
        fromBlock: 18500000,
        toBlock: 18501000,
      }),
  ...
  it('should handle different concurrency settings', async () => {
    ...
    const [highConcurrencyEvents, lowConcurrencyEvents] = await Promise.all([
      highConcurrencyFetcher.fetchEvents(usdcContract, 'Transfer', {
        contractAddress: usdcContract.address,
        ...blockRange,
      }),
      lowConcurrencyFetcher.fetchEvents(usdcContract, 'Transfer', {
        contractAddress: usdcContract.address,
        ...blockRange,
      }),
    ]);

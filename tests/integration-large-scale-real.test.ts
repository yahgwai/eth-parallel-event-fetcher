...
describe('Integration - Large Scale Real', () => {
  ...
  it('should demonstrate large-scale processing capability with realistic event density', async () => {
    ...
    const step1Events = await fetcher.fetchEvents(usdcContract, 'Transfer', {
    ...
    const step2Events = await fetcher.fetchEvents(usdcContract, 'Transfer', {
      contractAddress: usdcContract.address,
      fromBlock: 18501000,
      toBlock: 18511000,
    });
    ...
    const step3Events = await fetcher.fetchEvents(usdcContract, 'Transfer', {
      contractAddress: usdcContract.address,
      fromBlock: 18511000,
      toBlock: 18561000,
    });
    ...
    const step4Events = await fetcher.fetchEvents(usdcContract, 'Transfer', {
      contractAddress: usdcContract.address,
      fromBlock: 18561000,
    ...
    console.log(
      `   ✅ Found ${step4Events.length} events in ${remainingBlocks.toLocaleString()} blocks`
    );
    ...
    if (totalEvents >= 50000) {
      ...
      console.log(
        `   📊 Successfully demonstrated processing ${totalBlocks.toLocaleString()} blocks`
      );

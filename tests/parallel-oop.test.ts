...
describe('ParallelExecutor', () => {
  ...
  describe('successful execution', () => {
    ...
    test('should call progress callback', async () => {
      ...
      expect(onProgressMock).toHaveBeenCalledWith(expect.any(Number), 3);
  ...
  describe('error handling', () => {
    ...
    test('should throw after exhausting retries when continueOnError is false', async () => {
      const failingTask = jest.fn().mockRejectedValue(new Error('Persistent failure'));

      await expect(executor.execute([failingTask])).rejects.toThrow('Persistent failure');
  ...
  describe('rate limiting', () => {
    test('should handle rate limit errors with appropriate delays', async () => {
      ...
      const task = jest.fn().mockRejectedValueOnce(rateLimitError).mockResolvedValueOnce('success');
...
describe('Integration with executeInParallel', () => {
  test('should maintain backward compatibility', async () => {
    ...
    const tasks = [jest.fn().mockResolvedValue('result1'), jest.fn().mockResolvedValue('result2')];

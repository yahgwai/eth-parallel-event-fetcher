import { 
  executeInParallel, 
  chunkArray, 
  createBlockRangeChunks 
} from '../src/utils/parallel';
import { 
  createProvider, 
  getProviderStatus 
...
describe.skip('Parallel Utils', () => {
  describe('createBlockRangeChunks', () => {
    ...
    test('should throw error for invalid start/end blocks', () => {
      expect(() => createBlockRangeChunks(200, 100, 50))
        .toThrow(BlockRangeError);
      
      expect(() => createBlockRangeChunks(200, 100, 50))
        .toThrow('Start block must be less than or equal to end block');
    });

    test('should throw error for invalid chunk size', () => {
      expect(() => createBlockRangeChunks(100, 200, 0))
        .toThrow(BlockRangeError);
      
      expect(() => createBlockRangeChunks(100, 200, 0))
        .toThrow('Chunk size must be positive');
      
      expect(() => createBlockRangeChunks(100, 200, -10))
        .toThrow(BlockRangeError);
  ...
  describe('executeInParallel', () => {
    ...
    test('should throw on first error when continueOnError is false', async () => {
      ...
      await expect(executeInParallel(tasks, { 
        showProgress: false,
  ...
  describe('ProgressTracker', () => {
    ...
    test('should log progress when forced', () => {
      ...
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('Test: 25/100 (25.00%)')
      );
    });
    ...
    test('should log when reaching 100%', () => {
      tracker.update(100);
      expect(logSpy).toHaveBeenCalledWith(
        expect.stringContaining('Test: 100/100 (100.00%)')
      );
    });
